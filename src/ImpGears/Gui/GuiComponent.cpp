...
bool GuiComponent::event(imp::State* state, float x, float y)
{
	...
	for(unsigned int i=0; i<_components.size(); ++i)
	{
		...
		if( result == false && _components[i]->isVisible() )
		{
	...
	if(result == false)
	{
		...
		bool pressed = false;
		State::ActionState pressedState = State::ActionState_False;
		int button = 0;
		if( state->m_pressedMouseButtons[imp::Event::Mouse_LeftButton] == State::ActionState_Pressed
			|| state->m_pressedMouseButtons[imp::Event::Mouse_LeftButton] == State::ActionState_True)
		{
			button = 0;
			pressed = true;
			pressedState = state->m_pressedMouseButtons[imp::Event::Mouse_LeftButton];
		}
		else if( state->m_pressedMouseButtons[imp::Event::Mouse_RightButton] == State::ActionState_Pressed
			|| state->m_pressedMouseButtons[imp::Event::Mouse_RightButton] == State::ActionState_True)
		{
			button = 1;
			pressed = true;
			pressedState = state->m_pressedMouseButtons[imp::Event::Mouse_RightButton];
		}
		...
		for(unsigned int i=0; i<_eventHandlers.size(); ++i)
		{
			...
			if( pressed == true && _previousEventState._pressed == false )
				result |= _eventHandlers[i]->onMousePressed(this, mouseOver, button, mouseX, mouseY, (pressedState == State::ActionState_Pressed) );
			if( pressed == false && _previousEventState._pressed == true )
				result |= _eventHandlers[i]->onMouseReleased(this, mouseOver, _previousEventState._button, mouseX, mouseY, (pressedState == State::ActionState_Released));
