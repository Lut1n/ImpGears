...
static std::uint32_t s_frameBufferCount = 0;
static FrameBuffer* s_frameBufferBound = nullptr;
...
{
    glBindFramebuffer(GL_FRAMEBUFFER, _id);
    s_frameBufferBound = this;
}

//--------------------------------------------------------------
void FrameBuffer::unbind()
{
    s_frameBufferBound = nullptr;
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
}
...
{
    if(s_frameBufferBound == nullptr) return;

    std::vector<unsigned int>& drawBuffers = s_frameBufferBound->_drawBuffers;
    std::vector<Vec4>& clearColors = s_frameBufferBound->_clearColors;
...
FrameBuffer* FrameBuffer::s_bound()
{
    return s_frameBufferBound;
}

std::uint32_t FrameBuffer::s_count()
{
    return s_frameBufferCount;
}
