...
void LayeredImage::build(int w, int h, int chnlCount, int chnlSize)
{
	...
	{
		if(chnlSize == b8)
			_layers[i] = ChannelLayer<b8_t>::create(Dimension<2>(h,w));
		if(chnlSize == b16)
			_layers[i] = ChannelLayer<b16_t>::create(Dimension<2>(h,w));
		if(chnlSize == b32)
			_layers[i] = ChannelLayer<b32_t>::create(Dimension<2>(h,w));
		if(chnlSize == b64)
			_layers[i] = ChannelLayer<b64_t>::create(Dimension<2>(h,w));
	}
}
...
std::uint64_t LayeredImage::get(int x, int y, int ch) const
{
	...
	if(ls == b8) return getLayer<b8_t>(ch)->get(Dimension<2>(y,x));
	if(ls == b16) return getLayer<b16_t>(ch)->get(Dimension<2>(y,x));
	if(ls == b32) return getLayer<b32_t>(ch)->get(Dimension<2>(y,x));
	if(ls == b64) return getLayer<b64_t>(ch)->get(Dimension<2>(y,x));
...
{
	int ls = _layers[ch]->size();
	if(ls == b8) getLayer<b8_t>(ch)->set(Dimension<2>(y,x),val);
	if(ls == b16) getLayer<b16_t>(ch)->set(Dimension<2>(y,x),val);
	if(ls == b32) getLayer<b32_t>(ch)->set(Dimension<2>(y,x),val);
	if(ls == b64) getLayer<b64_t>(ch)->set(Dimension<2>(y,x),val);
}
