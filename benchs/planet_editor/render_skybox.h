    ...
    {    
        ...
        *skyGeo->_geo = Geometry::cube();
        ...
        _shaderGen = ReflexionModel::create(
                    ...
                    ReflexionModel::MRT_2_Col_Emi, "glsl skygen");
        _shaderGen->_fragCode_lighting = glsl_gen;
        _shaderGen->_vertCode = skyVert;

        skyGeo->setReflexion(_shaderGen);
        // skyGeo->getState()->getRenderPass()->enablePass(RenderPass::Pass_EnvironmentMapping);
        // this->getState()->getRenderPass()->enablePass(RenderPass::Pass_EnvironmentMapping);
        
        // float reversed = 0.0;
        // skyGeo->getState()->setUniform("u_reversed", reversed);
        // skyGeo->getState()->setUniform("u_input_cube", _cubemap, 3);
    ...
    SkyBox() : GeoNode(Geometry::create(), false)
    {
        *_geo = Geometry::sphere(8,50.0);
        ...
        _shader = ReflexionModel::create(
                    ReflexionModel::Lighting_Customized,
                    ReflexionModel::Texturing_Samplers_CNE,
                    ReflexionModel::MRT_2_Col_Emi, "glsl skybox");
