...
#include <RendererGL/Texture.h>
#include <RendererGL/GLInterface.h>
...
//--------------------------------------------------------------
void GlPlugin::init(void* glLoaderFunc)
{
    if(internalState == nullptr)
    {
        std::cout << "GPU Renderer init" << std::endl;
        internalState = new Priv();
        
        if (!gladLoadGLLoader(reinterpret_cast<GLADloadproc>(glLoaderFunc)))
        {
            std::cout << "Error: glad loader" << std::endl;
            exit(0);
        }
        ...
        internalState->_needInverseMat = true; // (major<3) || (major==3 && minor==0);
    }
}
...
int GlPlugin::load(ReflexionModel::Ptr& program)
{
    ...
    if(internalState->_needInverseMat)
    {
