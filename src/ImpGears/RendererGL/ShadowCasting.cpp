...
vec3 unproject(vec2 txCoord, float depth)
{
    ...
    ray = normalize(ray);
    float view_depth = impNear + depth * (impFar - impNear);
              ...
              out float out_depth)
{
    ...
    float impNear = 0.1;
    float impFar = 128.0;
    float distance_light = (length(pos_from_light)- impNear) / (impFar - impNear);
    ...
    // distance_light = (length(pos_from_light)-impNear) / (50.0-impNear);
...
void ShadowCasting::setup(const Vec4& vp)
{
    ...
    _fillingGraph = buildQuadGraph("glsl_fill", getFillGlsl(), viewport);
